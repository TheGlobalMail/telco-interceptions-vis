...
function htmlThatTable(tableFile, cb){
  var htmlFile = tableFile.replace(/csv$/, 'html');
  var html = styles;
  var heading = '';
  ...
  csv()
    ...
    .on('record', function(row, index){
      ...
      html += _.map(row.slice(1), function(cell){
    ...
    .on('end', function(){
      html += '</tbody></table>';
      fs.writeFile(path.join(dir, htmlFile), html, cb);
    });
}

async.forEach(['auth-exist-table.csv', 'auth-protect-table.csv'], htmlThatTable, function(err){
  if (err){
